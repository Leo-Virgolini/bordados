...
export class CustomersTabComponent implements OnInit {
    ...
    openCustomerDialog(customer?: Customer): void {
        ...
        if (customer) {
            ...
            // Disable ID field for new customers (will be generated by backend)
            this.customerForm.get('id')?.disable();
        }
    ...
    saveCustomer(): void {
        if (this.customerForm.valid) {
            ...
            const formValue = this.customerForm.getRawValue(); // Use getRawValue to get disabled field values
            ...
            if (this.editingCustomer) {
                ...
                this.customersService.updateCustomer(updatedCustomer).subscribe({
                    next: (customer) => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Actualizado',
                            detail: `Cliente ID: ${customer.id} actualizado correctamente`
                        });
                ...
                // Create new customer - don't send ID (will be generated by backend)
                const { id, ...customerData } = formValue;
                this.customersService.createCustomer(customerData).subscribe({
                    next: (newCustomer) => {
                        ...
                        this.messageService.add({
                            severity: 'success',
                            summary: 'Creado',
                            detail: `Cliente ID: ${newCustomer.id} creado correctamente`
                        });
                        this.showCustomerDialog = false;
                        this.editingCustomer = null;
                        this.customerLoading = false;
